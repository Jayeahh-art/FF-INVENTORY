...
function showApp() {
  ...
  document.getElementById('refresh-btn').onclick = () => router(true);
  router();
}
...
function router(forceRefresh) {
  ...
  v.innerHTML = `<div class="flex items-center gap-2 text-slate-500"><div class="spinner"></div> Loading…</div>`;
  Promise.resolve()
    .then(() => route.view(v, forceRefresh === true))
    .catch(err => {
...
async function viewInventory(root) {
  const rows = await api('inv.onHand');
...
async function openAdjustModal(itemId) {
  const item = await api('items.get', { id: itemId });
  const html = `
...
async function viewLowStock(root) {
  const rows = await api('items.lowStock');
  root.innerHTML = `
    <div class="bg-white rounded-xl shadow-sm border border-slate-200">
...
async function openItemModal(id) {
  const existing = id ? await api('items.get', { id }) : {};
  const html = `
...
async function deleteItem(id) {
  ...
  try { await api('items.delete', { id }); toast('Deleted', 'success'); router(true); }
  catch (e) { toast(e.message, 'error'); }
}
...
async function openSupplierModal(id) {
  const e = id ? await api('suppliers.get', { id }) : {};
  const html = `
    <div class="grid grid-cols-2 gap-3">
...
async function deleteSupplier(id) {
  ...
  try { await api('suppliers.delete', { id }); toast('Deleted', 'success'); router(true); }
  catch (e) { toast(e.message, 'error'); }
}

// ---------- Purchase Orders ----------

async function viewPOs(root) {
  const rows = await api('po.list');
  root.innerHTML = `
    <div class="flex justify-between items-center mb-4">
...
async function openPOEditor(id) {
  const [suppliers, items] = await Promise.all([api('suppliers.list'), api('items.list')]);
  ...
  await modal(html, {
    ...
    onMount: (m, close) => {
      ...
      if (sendBtn) sendBtn.onclick = async () => {
        try { await api('po.send', { id: po.id }); toast('PO marked sent', 'success'); close(true); router(true); }
        catch (e) { toast(e.message, 'error'); }
      };
      ...
      if (cancelBtn) cancelBtn.onclick = async () => {
        ...
        try { await api('po.cancel', { id: po.id }); toast('Cancelled', 'success'); close(true); router(true); }
        catch (e) { toast(e.message, 'error'); }
      };
    }
  });
...
async function viewReceiving(root) {
  const pos = (await api('po.list')).filter(p => p.status === 'sent' || p.status === 'partial');
  root.innerHTML = `
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <div class="px-5 py-3 border-b border-slate-200">
...
async function viewLedger(root) {
  const [txns, items] = await Promise.all([api('inv.ledger', { limit: 500 }), api('items.list')]);
  root.innerHTML = `
    <div class="flex justify-between items-center mb-4">
  ...
  document.getElementById('ledger-item').onchange = render;
  render();
}

// ---------- Stock Counts ----------

async function viewCounts(root) {
  const list = await api('count.list');
  root.innerHTML = `
    <div class="flex justify-between items-center mb-4">
  ...
  if (can('manager')) document.getElementById('new-count').onclick = async () => {
    ...
    try { const r = await api('count.create', {}); toast('Count started', 'success'); openCountModal(r.id); router(true); }
    catch (e) { toast(e.message, 'error'); }
  };
    ...
    onMount: (m, close) => {
      ...
      if (editable) {
        ...
        if (cancBtn) cancBtn.onclick = async () => {
          ...
          try { await api('count.cancel', { id }); toast('Cancelled', 'success'); close(true); router(true); }
          catch (e) { toast(e.message, 'error'); }
        };
      }
    }
...
async function deleteUser(id) {
  ...
  try { await api('users.delete', { id }); toast('Removed', 'success'); router(true); }
  catch (e) { toast(e.message, 'error'); }
}
